...
//点触事件
    ...
    Layer.prototype.view = function(){
        ...
        if(!config.type || config.type === 2){
            ...
            if(dialen >= 1){
                that.close(dialogs[0].getAttribute('index'))
            }
        }
    ...
    Layer.prototype.action = function(config, elem){
        ...
        var btn = function(){
            ...
            if(type == 0){
                ...
                config.yes ? config.yes(that.index) : that.close(that.index);
            }
        };
        if(config.btn){
        ...
        if(config.shade && config.shadeClose){
            ...
            ready.touch(shade, function(){
                that.close(that.index, config.end);
            });
        }
    ...
    var o;   //000058 change 2016-12-20 19:26:30
    win.layerApp = {
        v: '2.0',
        index: index,
        ...
        open: function(options){
            o = new Layer(options || {}); //000058 change 2016-12-20 19:26:30
            return o.index;
        },

        close: function(index){
            o.close(index); //000058 change 2016-12-20 19:26:30
        },
        ...
        closeAll: function(){
            ...
            for(var i = 0, len = boxs.length; i < len; i++){
                _this.close((boxs[0].getAttribute('index')|0));
            }
        }
    };
